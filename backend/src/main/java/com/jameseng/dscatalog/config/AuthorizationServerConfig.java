...
public class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		...
		.withClient("dscatalog")						//difinir o cliente id
		.secret(passwordEncoder.encode("dscatalog123"))	//definir a senha da aplicação: dscatalog123;
		.scopes("read", "write")						//tipo de acesso: litura e escrita
		.authorizedGrantTypes("password")
		.accessTokenValiditySeconds(86400);				//tempo de expiração do Token (em segundos) | 86400 = 24 horas
	}
